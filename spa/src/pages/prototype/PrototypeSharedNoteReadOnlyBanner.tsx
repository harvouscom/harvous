...
export type SharedNoteEditStatus =
  ...
  | { kind: 'offline' }
  /** Co-editable and nobody is writing — this viewer may take the pen. */
  | { kind: 'available' }
...
function statusCopy(status: SharedNoteEditStatus): { icon: string; label: string } {
  switch (status.kind) {
    ...
    case 'reconnecting':
      return { icon: 'eye', label: 'Reconnecting — your changes are saved locally' };
    case 'read-only':
    default:
...
export default function PrototypeSharedNoteReadOnlyBanner({
  ...
  return (
    <div className="proto-shared-readonly-banner" role="status" aria-live="polite">
      ...
      {status.kind === 'available' && onTakePen ? (
        <button
          type="button"
