...
// Helper function to remove UUID/hash patterns from filenames while preserving meaningful text
function stripAutoGeneratedParts(filename: string): string {
  if (!filename) return '';
  
  let cleaned = filename.trim();
  
  // Remove UUID patterns (with or without dashes/spaces)
  // Pattern: 8-4-4-4-12 hex characters, can have dashes, spaces, or be continuous
  // Examples: "651e425c-0742-4891-8cd9-ff31b686311c" or "651e425c 0742 4891 8cd9 ff31b686311c"
  cleaned = cleaned.replace(/\b[0-9a-f]{8}[- ]?[0-9a-f]{4}[- ]?[0-9a-f]{4}[- ]?[0-9a-f]{4}[- ]?[0-9a-f]{12}\b/gi, '');
  
  // Remove hash patterns (32+ consecutive hex characters)
  ...
  try {
    ...
    if (isPDF) {
      ...
      if (lastSegment) {
        ...
        let decodedFilename = decodeURIComponent(lastSegment)
          .replace(/\.pdf$/i, '')  // Case-insensitive, only at end
          .replace(/[-_]/g, ' ')
          .trim();
        
        // Remove UUID/hash patterns while preserving meaningful text
        decodedFilename = stripAutoGeneratedParts(decodedFilename);
        
        // Only use the filename if there's meaningful text left after stripping
        ...
        // If no meaningful text remains, filename stays as 'PDF Document'
      }
