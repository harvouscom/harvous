...
/** Matches GET /api/notes/:id/details `tags[]` payload (narrow fields used in UI). */
interface NoteTagDetail {
  id: string;
  name: string;
  color?: string | null;
  category?: string | null;
  isSystem?: boolean;
  /** From NoteTags junction: keyword/suggested tagging vs user-assigned */
  isAutoGenerated?: boolean | null;
  confidence?: number | null;
}

function normalizeNoteTag(raw: unknown): NoteTagDetail {
  ...
  const ig = r.isAutoGenerated;
  let isAutoGenerated: boolean | null = null;
  if (ig === true) isAutoGenerated = true;
  else if (ig === false) isAutoGenerated = false;
  return {
    id,
    name,
    color: typeof r.color === 'string' ? r.color : r.color === null ? null : undefined,
    category: typeof r.category === 'string' ? r.category : r.category === null ? null : undefined,
    isSystem: typeof r.isSystem === 'boolean' ? r.isSystem : undefined,
    isAutoGenerated,
    confidence: typeof r.confidence === 'number' ? r.confidence : r.confidence === null ? null : undefined,
  };
}

function normalizeTagsFromApi(tags: unknown): NoteTagDetail[] {
  if (!Array.isArray(tags)) return [];
  return dedupeNoteTagsByName(tags.map(normalizeNoteTag).filter((t) => t.id.length > 0));
}
...
export default function NoteDetailsPanel({
  ...
  const [localTags, setLocalTags] = useState<NoteTagDetail[]>(() => normalizeTagsFromApi(tags));
  ...
  const canEditOwnedNote = noteUserId
    ? Boolean(effectiveCurrentUserId && noteUserId === effectiveCurrentUserId)
  ...
  useEffect(() => {
    ...
    if (cached) {
      ...
      setLocalTags(normalizeTagsFromApi(cached.tags));
  ...
  const fetchNoteDetails = async (preserveTab: boolean = false, backgroundRefetch: boolean = false) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setLocalTags(normalizeTagsFromApi(data.tags));
        if (data.note) {
  ...
  const removeTagFromNote = async (tag: Pick<NoteTagDetail, 'id' | 'name' | 'isAutoGenerated'>) => {
    try {
      const label = tag.name?.trim() || 'Tag';
      const response = await fetch(
        `/api/note-tags/remove-by-name?noteId=${encodeURIComponent(noteId)}&name=${encodeURIComponent(label)}`,
        {
          method: 'DELETE',
          credentials: 'include',
        },
      ...
      if (response.ok) {
        toast.success(
          tag.isAutoGenerated === true
            ? `Removed suggested tag "${label}" from this note`
            : `Removed "${label}" from this note`,
        );
        ...
        const error = await response.json();
        toast.error(error.error || 'Could not remove tag from this note');
      }
    } catch {
      toast.error('Could not remove tag from this note. Please try again.');
    }
  };
  ...
  return (
    <>
      ...
      <style>{`
        ...
        /* Tag remove control (button) — scale on hover; z-index lives in panels.css */
        .note-details-panel .tag-item .note-details-panel__tag-remove {
          display: flex;
          align-items: center;
          justify-content: center;
        }

        .note-details-panel .tag-item .note-details-panel__tag-remove:hover {
                  ...
                  <div className="tab-content__section flex-1 min-h-0 flex flex-col">
                    ...
                    {activeTab === 'tags' && (
                      <div className="tab-content__section note-details-panel__tags-tab flex-1 min-h-0 flex flex-col">
                        ...
                        >
                          <div className="note-details-panel__tags-body">
                            {localTags.length === 0 ? (
                              <div className="panel__empty-state-with-description">
                                <p>No tags on this note yet</p>
                                <p className="panel__empty-state-description">
                                  {canEditOwnedNote ? (
                                    <>
                                      Harvous suggests tags from your saved content — use the ✕ on a chip to remove a suggestion, or type your own tag below (it attaches only to this note).
                                    </>
                                  ) : (
                                    <>
                                      Tags are shown here for reference; only the note owner can edit them.
                                    </>
                                  )}
                                </p>
                              </div>
                              ...
                              <div className="tag-list">
                                {localTags.map((tag: NoteTagDetail) => (
                                  <div key={tag.id} className="content-item tag-item">
                                    <div className="relative nav-item-container">
                                      <div className="btn btn--tag group w-auto max-w-full" title={tag.isAutoGenerated === true ? 'Suggested tag' : undefined}>
                                        <div className="btn__content note-details-panel__tag-chip-contents">
                                          <span className="note-details-panel__tag-name whitespace-nowrap">{tag.name}</span>
                                          {tag.isAutoGenerated === true ? (
                                            <span className="note-details-panel__tag-kind note-details-panel__tag-kind--suggested">
                                              Suggested
                                            </span>
                                          ) : null}
                                        </div>
                                      ...
                                      {/* Remove from note — only for notes the current user owns */}
                                      {canEditOwnedNote && (
                                        <button
                                          type="button"
                                          className="note-details-panel__tag-remove absolute top-1/2 right-3 transform -translate-y-1/2 flex-center w-4 h-4 cursor-pointer"
                                          aria-label={
                                            tag.isAutoGenerated === true
                                              ? `Remove suggested tag ${tag.name} from this note`
                                              : `Remove tag ${tag.name} from this note`
                                          }
                                          data-item-id={tag.id}
                                        ...
                                        >
                                          <svg className="w-4 h-4 fill-current" style={{ color: 'var(--color-deep-grey)' }} viewBox="0 0 384 512" aria-hidden>
                          ...
                          {canEditOwnedNote && (
                            <div className="note-details-panel__tags-compose-sticky">
                              <div className="note-details-panel__tags-compose-intro" aria-live="polite">
                                <p className="note-details-panel__tags-compose-heading">Add your tag</p>
                                <p className="note-details-panel__tags-compose-caption">
                                  Adds a tag to your library if it&apos;s new, then attaches it to this note.
                                </p>
                              </div>
                              <NewTagPanel
                                noteId={noteId}
                        ...
                        {isHarvousAdmin && canEditOwnedNote && (
