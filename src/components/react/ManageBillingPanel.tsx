                        ...
                        <div className="min-w-0 flex-1 flex justify-between items-center text-left">
                          <span className="text-base font-semibold" style={{ color: notesAtLimit ? limitRed : 'var(--color-deep-grey)' }}>
                            {subscriptionInfo.hasUnlimited
                              ? 'Unlimited notes'
                              : `${subscriptionInfo.currentCount.toLocaleString()} of ${(subscriptionInfo.limit ?? 1000).toLocaleString()} notes${(subscriptionInfo.referralBonusNotes ?? 0) > 0 ? ` (+${subscriptionInfo.referralBonusNotes} from referrals)` : ''}`}
                          </span>
                          {!subscriptionInfo.hasUnlimited && (
