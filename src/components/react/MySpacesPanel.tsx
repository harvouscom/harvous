...
export default function MySpacesPanel({ 
  ...
  const renderSpaceItem = (space: DisplaySpace) => {
    const spaceAccentColor = space.color ? getThreadColorCSS(space.color) : "var(--color-paper)";
    const isJoined = joinedSpaceIds.has(space.id);
    const isOwnedShared = sharedSpaceIds.has(space.id);
    // Same icon tint as CardThread / CondensedThreadItem (color-mix on thread token; paper bar → grey icon).
    const iconTintColor = space.color ?? 'paper';
      ...
      <div key={space.id} className="relative group">
        ...
        >
          ...
          <div className="flex items-center gap-6 pl-3 pr-4 h-full">
            {/* Private: single user. Owned shared: group. Joined (member): user-check — icon tint matches CardThread */}
            <div
              className="relative shrink-0 size-5 thread-accent-icon"
              style={{ ['--thread-accent-icon-color' as string]: getThreadIconOnAccentCSS(iconTintColor) }}
            >
              {isJoined ? (
                <svg className="block max-w-none size-full" fill="currentColor" viewBox="0 0 640 512" aria-hidden="true">
                ...
                <svg className="block max-w-none size-full" fill="currentColor" viewBox="0 0 640 640" aria-hidden="true">
                ...
                <svg className="block max-w-none size-full" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
