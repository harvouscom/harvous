/**
 * The text half of inline reference suggestions: providers, the word scan, and passage HTML
 * decoration. No TipTap here on purpose.
 *
 * The Bible reader and the scripture dock use these on plain passage HTML, and they used to
 * import them from `TiptapReferenceSuggestion.ts` — which pulled TipTap into every route's
 * initial payload through the reader, whether or not an editor ever opened. The extension
 * itself stays there and re-exports all of this, so editor-side imports did not change.
 */
