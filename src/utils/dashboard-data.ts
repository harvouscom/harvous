...
export async function getContentItems(userId: string, limit = 20, offset = 0, filterExcludeReferencedScripture = false, threads?: any[]) {
  try {
    ...
    const threadItems = threadsToUse.map(thread => ({
      ...
      lastVisited: thread.lastVisited || null, // Preserve null to distinguish visited from unvisited items in sorting
      createdAt: thread.createdAt,
    ...
    const assignedNoteItems = assignedNotes.map(note => {
      ...
      return {
        ...
        lastVisited: note.lastVisited || null, // Preserve null to distinguish visited from unvisited items in sorting
