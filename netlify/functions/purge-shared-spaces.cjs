...
var ts, Spaces, Threads, ThreadProgress, Notes, NoteVersions, SpaceNotes, NoteThreads, StudyThreadEntries, SyncDeletedEntities, Comments, Members, SpaceInvitations, SpaceMemberships, SpaceInvites, Churches, ChurchMemberships, ChurchServiceTimes, ChurchServiceTimeAssignments, ChurchSeries, ChurchServices, NoteTemplates, UserMetadata, Entitlements, ClerkUserMapping, UserXP, UserSeasonalXP, UserLifetimeXP, WeeklyStreaks, Tags, NoteTags, ScriptureMetadata, NoteScriptureReferences, NoteFingerprints, RecallEvents, ReadingEvents, SupportTickets, SupportTicketNotes, DiagnosticEvents, DiagnosticIssueTriage, NoteConnections, StudyThreadMemberOrders, VerseTextCache, BibleTranslations, BibleVerses, ScriptureCrossReferences, ScriptureTopics, ScriptureTopicVerses, BiblePeople, BiblePlaces, ScriptureEntityRefs, TopicRelations, ResourceMetadata, ResourceLibraries, LibraryItems, LibraryItemScopes, LibraryItemSpacePins, LibraryItemSuggestions, ChurchServicePublishedNotes, ChurchSeriesPublishedNotes, ChurchServiceLibraryItems, ChurchSpaceChannelLinks, InboxItems, InboxItemNotes, UserInboxItems, FeaturedItems, VotdSchedule, VotdPublishHistory, UserFeaturedItems, MonthlyAnalytics, AdminMonthlyReports, ImportSessions, ImportSessionItems, AppSyncCursors;
var init_schema2 = __esm({
  "server/db/schema.ts"() {
    ...
    SpaceInvitations = pgTable("SpaceInvitations", {
      id: text("id").primaryKey(),
      spaceId: text("spaceId").notNull(),
    ...
    SpaceMemberships = pgTable("SpaceMemberships", {
      id: text("id").primaryKey(),
      spaceId: text("spaceId").notNull(),
      userId: text("userId").notNull(),
      /**
    ...
    SpaceInvites = pgTable("SpaceInvites", {
      id: text("id").primaryKey(),
      spaceId: text("spaceId").notNull(),
    ...
    ChurchMemberships = pgTable("ChurchMemberships", {
      id: text("id").primaryKey(),
    ...
    UserMetadata = pgTable("UserMetadata", {
      id: text("id").primaryKey(),
    ...
    UserSeasonalXP = pgTable("UserSeasonalXP", {
      id: text("id").primaryKey(),
      userId: text("userId").notNull(),
    ...
    UserLifetimeXP = pgTable("UserLifetimeXP", {
      id: text("id").primaryKey(),
      userId: text("userId").notNull().unique(),
      totalXP: integer("totalXP").notNull().default(0),
      lastUpdated: ts("lastUpdated").notNull()
    });
    WeeklyStreaks = pgTable("WeeklyStreaks", {
      id: text("id").primaryKey(),
      userId: text("userId").notNull(),
    ...
    ScriptureMetadata = pgTable("ScriptureMetadata", {
      id: text("id").primaryKey(),
      noteId: text("noteId").notNull(),
    ...
    NoteScriptureReferences = pgTable("NoteScriptureReferences", {
      id: text("id").primaryKey(),
      noteId: text("noteId").notNull(),
    ...
    ReadingEvents = pgTable("ReadingEvents", {
      id: text("id").primaryKey(),
      userId: text("userId").notNull(),
    ...
    BibleTranslations = pgTable("BibleTranslations", {
      id: text("id").primaryKey(),
      name: text("name").notNull(),
    ...
    BibleVerses = pgTable("BibleVerses", {
      id: text("id").primaryKey(),
    ...
    BiblePeople = pgTable("BiblePeople", {
      id: text("id").primaryKey(),
      slug: text("slug").notNull().unique(),
      name: text("name").notNull(),
    ...
    BiblePlaces = pgTable("BiblePlaces", {
      id: text("id").primaryKey(),
      slug: text("slug").notNull().unique(),
      name: text("name").notNull(),
      aliases: text("aliases"),
    ...
    TopicRelations = pgTable("TopicRelations", {
      id: text("id").primaryKey(),
    ...
    ResourceMetadata = pgTable("ResourceMetadata", {
      id: text("id").primaryKey(),
      noteId: text("noteId").notNull(),
    ...
    ChurchServicePublishedNotes = pgTable(
    ...
    ChurchSeriesPublishedNotes = pgTable(
    ...
    ChurchSpaceChannelLinks = pgTable(
    ...
    InboxItemNotes = pgTable("InboxItemNotes", {
      id: text("id").primaryKey(),
    ...
    UserInboxItems = pgTable("UserInboxItems", {
      id: text("id").primaryKey(),
      userId: text("userId").notNull(),
      inboxItemId: text("inboxItemId").notNull(),
      status: text("status").notNull(),
...
// server/utils/space-note-associations.ts
var init_space_note_associations = __esm({
  "server/utils/space-note-associations.ts"() {
    "use strict";
  }
});

// server/utils/shared-space-lifecycle.ts
function isExpiredSpacePurgeCandidate(space, now2) {
  return Boolean(
    space.deletedAt && space.recoveryUntil && space.recoveryUntil < now2 && !space.isActive
  );
}
async function purgeExpiredDeletedSpaces(now2 = /* @__PURE__ */ new Date(), limit = EXPIRED_SPACE_PURGE_BATCH_SIZE) {
  return db.transaction(async (tx) => {
    const expired = await tx.select({ id: Spaces.id }).from(Spaces).where(
      and(
        isNotNull(Spaces.deletedAt),
        isNotNull(Spaces.recoveryUntil),
        lt(Spaces.recoveryUntil, now2),
    ...
    for (const { id: spaceId } of expired) {
      const locked = first(
        ...
        }).from(Spaces).where(eq(Spaces.id, spaceId)).for("update").limit(1)
      );
      if (!locked || !isExpiredSpacePurgeCandidate(locked, now2)) {
        continue;
      }
      const threadRows = await tx.select({ id: Threads.id }).from(Threads).where(eq(Threads.spaceId, spaceId));
...
var SPACE_RECOVERY_WINDOW_MS, EXPIRED_SPACE_PURGE_BATCH_SIZE, PURGE_MAINTENANCE_INTERVAL_MS;
var init_shared_space_lifecycle = __esm({
  "server/utils/shared-space-lifecycle.ts"() {
    "use strict";
    init_db2();
    init_space_note_associations();
    SPACE_RECOVERY_WINDOW_MS = 30 * 24 * 60 * 60 * 1e3;
    EXPIRED_SPACE_PURGE_BATCH_SIZE = 10;
    PURGE_MAINTENANCE_INTERVAL_MS = 60 * 60 * 1e3;
  }
});

// server/netlify-purge-shared-spaces.ts
var netlify_purge_shared_spaces_exports = {};
__export(netlify_purge_shared_spaces_exports, {
  SCHEDULED_PURGE_BATCH_SIZE: () => SCHEDULED_PURGE_BATCH_SIZE,
  SCHEDULED_PURGE_MAX_SPACES: () => SCHEDULED_PURGE_MAX_SPACES,
  SCHEDULED_PURGE_TIME_BUDGET_MS: () => SCHEDULED_PURGE_TIME_BUDGET_MS,
  createPurgeSharedSpacesHandler: () => createPurgeSharedSpacesHandler,
  default: () => netlify_purge_shared_spaces_default,
  handler: () => handler
});
module.exports = __toCommonJS(netlify_purge_shared_spaces_exports);
init_shared_space_lifecycle();
var SCHEDULED_PURGE_BATCH_SIZE = 50;
var SCHEDULED_PURGE_MAX_SPACES = 500;
var SCHEDULED_PURGE_TIME_BUDGET_MS = 25e3;
function createPurgeSharedSpacesHandler(purge = purgeExpiredDeletedSpaces, clock = Date.now) {
  return async function handler2() {
    ...
    } while (purged < SCHEDULED_PURGE_MAX_SPACES && clock() - startedAt < SCHEDULED_PURGE_TIME_BUDGET_MS);
...
var handler = createPurgeSharedSpacesHandler();
var netlify_purge_shared_spaces_default = handler;
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  SCHEDULED_PURGE_BATCH_SIZE,
  SCHEDULED_PURGE_MAX_SPACES,
  SCHEDULED_PURGE_TIME_BUDGET_MS,
  createPurgeSharedSpacesHandler,
  handler
});
