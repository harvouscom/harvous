...
route.get('/api/notes/:id/tags', requireAuth, async (c) => {
  try {
    ...
    // same reason as GET /api/note-tags/list.
    return c.json(
      {
        success: true,
...
route.get('/api/notes/:id/details', requireAuth, async (c) => {
  try {
    ...
    // same reason as GET /api/user/get-profile's override (see that route's comment). The
    // normal save path never re-hits this endpoint (useUpdateNote's onSuccess patches the
    // React Query cache directly with the server's response), but the 404/403 recovery path
    // in useUpdateNote.ts does call invalidateQueries(..., { refetchType: 'all' }) here, and
    // reopening a note edited moments ago in another tab/session hits this same GET — both
    // are exactly the shape of request the browser's HTTP cache would silently satisfy with
    // a pre-edit response instead of reaching the server.
...
route.get('/api/notes/:noteId/share', requireAuth, async (c) => {
  try {
    ...
    // Overrides app.ts's blanket cache default (see GET /api/user/get-profile's comment) —
    // this is a publish toggle's own status read; a cached response here can show a note as
    // still public after disabling sharing, or hide a just-generated share link.
    return c.json(
      {
