...
/** POST /api/resource/:id/update-image */
app.post('/api/resource/:id/update-image', async (c) => {
  try {
...
/** POST /api/resource/check-duplicate */
app.post('/api/resource/check-duplicate', async (c) => {
  try {
    const auth = getAuth(c);
    if (!auth.userId) return c.json({ error: 'Authentication required' }, 401);
...
/** POST /api/resource/suggest-threads */
app.post('/api/resource/suggest-threads', async (c) => {
  try {
    const auth = getAuth(c);
...
/** POST /api/resource/update-metadata */
app.post('/api/resource/update-metadata', async (c) => {
  try {
    const auth = getAuth(c);
    if (!auth.userId) return c.json({ error: 'Authentication required' }, 401);
