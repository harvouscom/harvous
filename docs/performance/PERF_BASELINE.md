...
A different axis from the payload above: **an Activity load fires 30 API requests**, down from 36
before the four duplicated reads in "Fixed" below were merged.

**Measured 2026-09-04**, signed in, warm cache, one reader with 30 notes, 4 suggestion cards and
an empty review queue. Reproduce by loading `/` and reading the browser's network log filtered to
`/api/`. Note that `performance.getEntriesByType('resource')` returns **nothing** for these —
something in the fetch path keeps them out of resource timing — so the network panel is the only
way to count them.

The count moves with what Home has to show: the same fixes measured 40 → 31 on
`fix/review-row-identity`, where that reader had 6 suggestion cards and 11 review items. Compare
loads, not branches.

Note also that reading this branch against the database requires
`UserMetadata.foundingClaimedAt` to exist — see `docs/RELEASE_CHECKLIST_3_0.md`. Without it
`/api/user/get-profile` 500s and Home never composes, so any count taken is a count of a page
that failed.

What is left after the merges is 30 distinct endpoints, which is a different kind of problem: not
one surface asking twice, but Home composing itself out of many small reads. Attacking that means
merging endpoints or deferring sections, not deduplicating — so it is a design question rather
than a bug.
...
36 requests became 30 on this branch. Four separate duplications, one fix each, and the shape
they had in common is that no single file looked wrong: every one of them was two correct callers
of the same data with nothing between them saying so.
