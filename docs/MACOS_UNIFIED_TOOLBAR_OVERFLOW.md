...
| Topic | Status |
|--------|--------|
| **Symptom** | AppKit toolbar **overflow** (“more” chevron) **flashing** on **sidebar expand** (especially `.detailOnly` → `.all`), not necessarily at steady-state widths. |
...
| **Primary fix** | **`animatedSplitVisibility`** on `MacRootView`: **instant** expand from `.detailOnly` via `Transaction.disablesAnimations = true`; **collapse** keeps default slide animation. |
