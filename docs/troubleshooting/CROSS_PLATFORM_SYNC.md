...
4. Create a note on Mac — upload within ~2s; web sidebar should update after push (web) or within ~10s pull (native) without manual refresh.
5. Delete on web — Mac/iOS lists should update after background sync or tab focus.
...
- **`/prototype` shell:** `initializeSync` does not run (no bootstrap into IndexedDB on load). Cross-device updates on web prototype rely on React Query (`staleTime` refetch, tab visibility, and debounced `refreshPrototypeLists` when another tab completes offline pull/push). Use **Prototype → Data → Refresh from server** for a full cache clear.
