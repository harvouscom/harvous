...
erDiagram
    ...
    THREADS ||--o{ NOTES : "primary thread"
    THREADS ||--o{ NOTETHREADS : "via junction"

    NOTES ||--o{ NOTETHREADS : "via junction"
...
| **Notes** | Individual content items | title, content, threadId, simpleNoteId, noteType |
| **NoteThreads** | Many-to-many: notes ↔ threads | noteId, threadId |
...
- Default fallback for notes without threads
- Hidden from UI, cannot be deleted
- Ensures notes always have a primary thread
